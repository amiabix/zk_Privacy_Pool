    ...
    fn update_tree_from_leaf(&mut self, leaf_index: u32) {
        ...
        for level in 1..=self.depth {
            ...
            let parent_hash = if current_index % 2 == 0 {
                // Left child
                zisk_hash_pair(current_hash, sibling_hash)
            } else {
                // Right child
                zisk_hash_pair(sibling_hash, current_hash)
            };
    ...
    fn update_root(&mut self) {
        ...
        while current_level.len() > 1 {
            ...
            for i in (0..current_level.len()).step_by(2) {
                ...
                let parent = zisk_hash_pair(left, right);
    ...
    pub fn verify_proof(&self, proof: &TornadoMerkleProof, leaf: [u8; 32]) -> bool {
        ...
        for i in 0..self.depth as usize {
            ...
            current_hash = if is_left {
                zisk_hash_pair(current_hash, sibling)
            } else {
                zisk_hash_pair(sibling, current_hash)
            };
        }
...
impl TornadoCommitmentHasher {
    ...
    pub fn new(secret: [u8; 32], nullifier_seed: [u8; 32]) -> Self {
        let nullifier = zisk_generate_nullifier(secret, nullifier_seed);
    ...
    pub fn generate_commitment(&self, value: u64, blinding: [u8; 32]) -> [u8; 32] {
        zisk_pedersen_commitment(value, blinding)
    }
    ...
    pub fn verify_commitment(
        ...
        zisk_verify_commitment(commitment, value, blinding)
    }
    ...
    pub fn verify_nullifier(&self, nullifier: [u8; 32]) -> bool {
        zisk_verify_nullifier(nullifier, self.secret, self.nullifier_seed)
    }
}
...
impl TornadoWithdrawalCircuit {
    ...
    pub fn verify(&self) -> bool {
        ...
        // Verify range proof
        if !zisk_range_proof(self.value) {
            return false;
        }
